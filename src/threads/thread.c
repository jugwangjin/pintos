               ...
               thread_func *function, void *aux) 
{
  ...
  /* yield, so if this thread's priority is high, run */
  thread_yield ();
...
thread_unblock (struct thread *t) 
{
  ...
  /* change to list_insert_ordered, to make list ordered  
...
thread_set_priority (int new_priority) 
{
  ...
  /* highest for scheduling if not highest */
  if(list_empty(&ready_list))
    highest_priority = 0;
  else
  {
    highest_thread = list_entry (list_begin (&ready_list), struct thread, elem);
    highest_priority = thread_get_priority_from_pointer(highest_thread);
  }

  thread_current ()->priority = new_priority;

  /* yield if not highest */
...
{
  struct thread *cur = thread_current ();
  return thread_get_priority_from_pointer (cur);
}
